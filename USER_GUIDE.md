...
- **Each entity's own AutoCAD layer is preserved**: every element is grouped into a `<g class="layer layer-...">` per layer (the exact original layer name is kept, escaped, in that group's `data-layer-name` attribute — the class name itself is sanitized, since layer names can contain spaces, `|`, `$`, etc.). This applies to top-level content and to a block definition's own internal content alike, so a block that itself spans more than one layer keeps that structure too. It's a `class`, not an `id`, because the same layer name can legitimately recur in more than one independently-built group (the top level, and each block definition) — a single selector like `.layer-muebles` finds every group for that layer everywhere, including inside every block
- **Every element gets an `id`** derived from its source AutoCAD entity's handle (the same persistent identifier the `LIST` command shows), prefixed `e` (e.g. `id="e38F8"`) — lets you trace an SVG shape back to the exact object it came from. A piece produced by `explode()` (which has no handle of its own) gets its source entity's id with an index suffix (e.g. `e38F8_2`). A nested block's sub-entity (which is flattened, and can be flattened more than once if the same sub-block is used in multiple places) gets a composed id — the containing occurrence's id plus its own handle — so ids stay unique across every occurrence
...
- **Stroke width:** Each entity's own AutoCAD lineweight (ByLayer/ByBlock resolved the same way color is) drives its `stroke-width` — AutoCAD lineweights are always specified in millimeters, converted here into the drawing's own units via `INSUNITS`, so it comes out correct whether the drawing is in mm, m, inches, etc. A lineweight of exactly 0 is floored to a thin-but-visible hairline rather than an invisible zero-width stroke
...
- **ByBlock lineweight inside a shared block definition** falls back to a plain default rather than varying per instance (unlike ByBlock color, which does vary per instance via CSS `currentColor`) — a known simplification, since ByBlock lineweight is far less common in practice than ByBlock color
